    ...
    def __init__(self, *args: Tuple[Any, ...], **kwargs: Dict[str, Any]) -> None:
        ...
        def initialize(params: InitializeParams) -> None:
            ...
            virtual_environment = Path(
                gettempdir()) / f'grizzly-vscode-{project_name}'
            ...
            if project_name not in self.projects:
                ...
                behave_runner_util.load_step_modules(
                    [str(root_path / 'features' / 'steps')])

                project = GrizzlyProject(
                    project_name, root_path, behave_step_registry.registry.steps)

                self.projects.update({project_name: project})

                from pprint import pformat
                self.logger.info(
                    pformat(project.steps, indent=2))
