...
class RegexPermutationResolver:
    ...
    def permute_tokens(self, tokens: SreParseTokens) -> List[str]:
        lists: List[List[str]]
        lists = [self.handle_token(token, cast(SreParseValue, value)) for token, value in tokens]
        output: List[str] = []
    ...
    def cartesian_join(self, input: List[List[str]]) -> Generator[List[str], None, None]:
        def rloop(sequence: List[List[str]], combinations: List[str]) -> Generator[List[str], None, None]:
            if len(sequence) > 0:
                for item in sequence[0]:
                    _combinations = combinations + [item]
